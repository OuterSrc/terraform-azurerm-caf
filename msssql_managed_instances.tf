...
module "mssql_mi_administrators" {
  ...
  group_name          = try(local.combined_objects_azuread_groups[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.azuread_group_key].display_name, [each.value].azuread_group_name, null)
}
